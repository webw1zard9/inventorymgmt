...
{{--                <h4>Remaining Eligible Qty: {{ ($batch->track_inventory ? floatval($location->remainingPoInventory($batch->id))." ".$batch->uom : "Unlimited") }}</h4>--}}

                <div class="row">

                    <div class="col-xl-6 col-lg-6">

                        {{ Form::model($batchLocationAggregate, ['class'=>'form-horizontal', 'url'=>route('batch-location-aggregate.update', $batchLocationAggregate->id)]) }}
                        {{ Form::hidden('_edit_batch_location_aggregate',true) }}
                        {{ method_field('PUT') }}

                        <div class="form-group">
                            {{ Form::label('name', 'Name') }}
                            {{ Form::text('location_batch_name', old('location_batch_name'), array('class' => 'form-control', 'placeholder' => 'Name')) }}
                        </div>
                            ...
                            <div class="col-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        @else
                                            {{ Form::text("new_unit_cost", $batchLocationAggregate->location_unit_price, ['id'=>'unit_price', 'class'=>'form-control','placeholder'=>'']) }}
                                            <input type="hidden" id="current_avg_price" value="{{ $batchLocationAggregate->location_unit_price }}" />
                                        @endif
                                    </div>

                                    <span class="help-block text-danger"><small>Current average cost per unit: <strong>{{ display_currency($batchLocationAggregate->location_unit_price) }}</strong></small></span>
