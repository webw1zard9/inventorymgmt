...
@section('content')
    ...
    <div class="row">
        ...
        @endif
            ...
            @if($sales_by_rep ?? false)

                <!-- TOTALS if there are more than one location -->
                @if($sales_by_rep->count() > 1)
                    <div class="col-lg-3 col-md-6">
                        <div class="widget-bg-color-icon card-box">
                            <div class="bg-icon bg-icon-success pull-left">
                                <i class="text-info" style="font-size: 22px">{{ $sales_by_rep->sum('count') }}</i>
                            </div>
                            <div class="text-right">
                                <p class="text-muted mb-0 font-16"><strong>Total</strong></p>
                                <h3 class="text-success "><b class="counter">{{ display_currency($sales_by_rep->sum('total'), 0) }}</b></h3>
                                @if($sales_by_rep->count())
                                    <h5 class="text-">AOV: {{ display_currency($sales_by_rep->sum('total') / $sales_by_rep->sum('count'), 0) }}</h5>
                                @endif
                            </div>
                            <div class="clearfix"></div>
                        </div>
                ...
                @foreach($sales_by_rep as $rep)

                    <div class="col-lg-3 col-md-6">
                        <div class="widget-bg-color-icon card-box">
                            <div class="bg-icon bg-icon-info pull-left">
                                <i class="text-info" style="font-size: 22px">{{ $rep->count }}</i>
                            </div>
                            <div class="text-right">
                                <p class="text-muted mb-0 font-16"><strong>{{ $rep->name }}</strong></p>
                                <h3 class="text-dark "><b class="counter">{{ display_currency($rep->total, 0) }}</b></h3>
                                @if($rep->count())
                                    <h5 class="text-">AOV: {{ display_currency($rep->total / $rep->count, 0) }}</h5>
                                @endif
                            </div>
                            <div class="clearfix"></div>
                        </div>
